...
class Auction(models.Model):
    ...
    highest_bid = models.PositiveBigIntegerField(
        default=0
    )  # highest bid with defaults set to 0, so less thatn starting bid
    ...
    creator = models.ForeignKey(
        User, on_delete=models.CASCADE, null=True, related_name="created_auctions"
    )
    winner = models.ForeignKey(
        User, on_delete=models.CASCADE, null=True, related_name="won_auctions"
    )

    def __str__(self):
        return f"ID: {self.id}, Title:{self.title}, Description:{self.description}, an is open of {self.is_open}, with a starting bid of {self.starting_bid}, a highest bid of {self.highest_bid}, an image url of {self.image_url}, and category:{self.category} the winner is {self.winner}"
...
class Bids(
    models.Model
):  # bid model that includes auction foreign key that links to auction id, bids, and user id
...
class Comments(models.Model):  # comment model has auction id, comments, and user id
    ...
    def __str__(self):
        return (
            f"Auction:{self.auction} and Comments:{self.comments} from User:{self.user}"
        )
...
class UserProfile(
    models.Model
):  # user profile model has the user id and then the watchlist
